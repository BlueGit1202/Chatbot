...
- If the total price is between £100 and £200, then the real total price = [£100 + (total - £100) * 0.9](don't send to customer this formula), rounded up to the nearest £5(it means real total price= [floor(realtotalprice/10)]*10+result(realtotalprice%10<5 then result=0 else realtotalprice%10>5 then result=5 )).
- If the total price is more than £200, then the real total price = [£200 + (total price - 200) * 0.75+£10](don't send to customer this formula), rounded up to the nearest £5(it means real total price= [floor(realtotalprice/10)]*10+result(realtotalprice%10<5 then result=0 else realtotalprice%10>5 then result=5 )). Do not send this formula and reduce whole proccess to calcualte like "+","-","*" etc to customers—say it's not allowed by Richard only say discounted price is xxx like this.
